$(function () {
    ...
    function tableHead(data,pageObj,$tbodys,$theTotalBox){
        ...
        var otherClass = accAdd(accMul(accMul(num1, 2), 134), 3416); //支付宝 美团 百度 饿了么 3平台优惠 +1400 -1200
        if (pageObj.type == 2) {
        ...
        //for (var i in data) {
            ...
            for(var y=0;y<thMap.length;y++){
                ...
                //if(ct[_str] ){
                    ...
                    _tempOther_rec +="<th class='report_num' data-type='"+_str+"'>"+parseFloat(pay_money).toFixed(2)+"</th>";
                    _tempOther_dis +="<th class='report_num' data-type='"+_str+"'>"+parseFloat(preferential_money).toFixed(2)+"</th>";
            ...
            content +=  '<tr class="total-tr">'+
                    '<th data-type="payid">'+'<b>合计</b>'+'</th>'+ //结账单号
                    '<th data-type="btn">'+'——'+'</th>'+
                    
                    '<th data-type="ordertime">'+'——'+'</th>'+ //结账时间
                    '<th class="hide" data-type="ordertime">'+'——'+'</th>'+ // 店铺
                    ...
                    '<th class="report_num">'+cal_data.alip_money+'</th>'+// class="report_num"收入
                    ...
                    '<th data-type="ordertime">'+'——'+'</th>'+ //开台时间
                    '<th data-type="tableid" >'+'——'+'</th>'+ //桌台名称
                    '<th data-type="ausername">'+'——'+'</th>'+ //结账员工
                    '<th class="report_num">'+parseFloat(ct.user_num)+'</th>'+           //就餐人数
                    '<th class="report_num">'+cal_data.use_money+'</th>'+ //人均消费
                    '<th data-type="is_unusual">'+'——'+'</th>'+//状态
    ...
    function CashierList (data,$tbodys,pageObj,$theTotalBox) {
        ...
        for (var i in data) {
            ...
            content +=  '<tr class="total-tr">'+
                        ...
                        '<td class="hide">'+data[i].shop_name+'</td>'+ //店铺


                        '<th class="report_num">'+cal_data.con_money+'</th>'+               //消费金额
                        '<th class="report_num">'+cal_data.res_money+'</th>'+               //实收金额

                        '<th class="report_num">'+parseFloat(data[i].cash).toFixed(2)+'</th>'+                 //现金收入
                        '<th class="report_num">'+parseFloat(data[i].card).toFixed(2)+'</th>'+                 //银行卡收入
                        '<th class="report_num">'+cal_data.wxp_money+'</th>'+//微信收入
                        '<th class="report_num">'+cal_data.alip_money+'</th>'+// class="report_num"收入
                        ...
                        '<th class="report_num">'+data[i].user_num+'</th>'+           //就餐人数
                        '<th class="report_num">'+cal_data.use_money+'</th>'+ //人均消费
