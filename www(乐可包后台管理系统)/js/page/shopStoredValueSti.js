$(function () {
    ...
    //添加切换事件
    $("#cardType_1,#cardType_2,#cardType_3,#cardType_5,#cardType_6,#cardType_7").click(function(){
        ...
        $("#cardTypeDate,#downloadInfor").removeClass('hide');
    ...
    function downloadSelect(selectType,$theContent,storedType) {
     ...
     // 储值通用统计（储值通用消费明细）
    function ConsumeList(thePage,searchObj) {
        var startDate = $("#start-date").val();
        var endDate = $("#end-date").val();
        ...
        //清单对应的分页标签
        var $searchPage= $("#nav2_search_page");
        ...
        for (var i in data) { 
           ...
           if(i=='ct'){
                ...
                content_1 += '<tr class="stores_title">'+
                        ...
                        '<td width="12%" class="report_text">'+data[i].pay_card_name+'<span class="spanLeft">'+(data[i].entity_id==null?'(储值账户)':'(实体卡)')+'</td>'+
                        '<td width="7%" class="report_num '+ ClassColor(data[i].pay_stored)+'">'+(data[i].pay_stored==0?'————':data[i].pay_stored)+'</td>'+
                        '<td width="7%" class="report_num '+ ClassColor(data[i].pay_principal)+'">'+(data[i].pay_principal==0?'————':data[i].pay_principal)+'</td>'+
                        '<td width="7%" class="report_num '+ ClassColor(data[i].order_stored)+'">'+(data[i].order_stored==0?'————':data[i].order_stored)+'</td>'+
                        '<td width="7%" class="report_num '+ ClassColor(data[i].order_principal)+'">'+(data[i].order_principal==0?'————':data[i].order_principal)+'</td>'+
    ...
    function CashierList (data,$theContent) {
        ...
        for (var i in data) {
            ...
            content +=  '<tr class="total-tr">'+
                ...
                '<td class="report_num total-tel" data-type="storedMoney">'+data[i].stored_money+'</td>'+  // 储值金额
                '<td class="report_num total-tel" data-type="giveMoney">'+data[i].give_money+'</td>'+      // 赠送金额
                '<td class="report_num total-tel">'+parseFloat(data[i].stored_num)+'</td>'+//售卖数量
                '<td class="report_num total-tel" data-type="cash">'+parseFloat(data[i].cash).toFixed(2)+'</td>'+  // 现金
                '<td class="report_num total-tel" data-type="card">'+parseFloat(data[i].card).toFixed(2)+'</td>'+  // 银行卡
                '<td class="report_num total-tel" data-type="free">'+parseFloat(data[i].free).toFixed(2)+'</td>'+  // 免单
                '<td class="report_num total-tel">'+(data[i].wxpay_shop == null ? '0' : parseFloat(data[i].wxpay_shop).toFixed(2))+'</td>'+    // 银台微信
                '<td class="report_num total-tel">'+(data[i].alipay_shop == null ? '0' : parseFloat(data[i].alipay_shop).toFixed(2))+'</td>'+  // 银台支付宝
                '<td class="report_num total-tel">'+parseFloat(data[i].wxpay).toFixed(2)+'</td>'+  // 微信支付
                '<td class="report_num total-tel">'+parseFloat(data[i].alipay).toFixed(2)+'</td>'+  // 支付宝支付
