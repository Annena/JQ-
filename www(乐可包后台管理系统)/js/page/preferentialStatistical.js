		...
		var StoredValueStatistical = {
			...
			init: function () {
				...
				if (sign == undefined && hierarchy == undefined) {
					...
					// 从缓存中取到要请求的值请求获取数据
					this.StoredData(stoData.start_date, stoData.end_date, stoData.shop_ids, stoData.promo_ids);
				}
				// 如果多个日期并且是第三层的话就不显示标题列的（操作）
				if (sign == 2 && hierarchy == 3) {
					...
					if (way == 2) {
						// 导航条显示
						$('#Navigation').html('<b id="stat">优惠方案统计 </b>>> <b id="statDis">'+stoData.start_date+'至'+stoData.end_date+'</b> >> <b id="statShop">'+stoData.shopName+' >> '+stoData.promoName);
					} else {
						// 导航条显示
						$('#Navigation').html('<b id="stat">优惠方案统计 </b>>> <b id="statDis">'+stoData.start_date+'至'+stoData.end_date+'</b> >> <b id="statShop">'+stoData.promoName+' >> '+stoData.shopName);
					}
					// 从缓存中取到要请求的值请求获取数据
					this.StoredData(stoData.start_date, stoData.end_date, stoData.shop_ids, stoData.promo_ids);
				} else {
			...
			StoredBind: function () {
				...
				$('#statDis').unbind('click').bind('click', function () {
					...
					Cache.set('preferentstat',preferentstat);
					if (way == 1) {
						window.location.replace('preferentialShopStat.html?sign=2&click=2&hierarchy=1');
					} else if (way == 2) {
						window.location.replace('preferentialPlanStat.html?sign=2&click=2&hierarchy=1');
					}
				});
				// 导航点击跳转回多个日期那个店的所有优惠方案或那个优惠方案所在的所有店
				$('#statShop').unbind('click').bind('click', function () {
					var preferentstat = {
						'start_date': stoData.start_date,
						'end_date': stoData.end_date,
						'shop_ids': way == 2 ? stoData.shop_ids : 'all',
						'promo_ids': way == 2 ? 'all' : stoData.promo_ids,
						'type': way,
						'name': way == 2 ? stoData.shopName : stoData.promoName
					}
					// 这个缓存是存储导航条日期的
					Cache.set('preferentstat',preferentstat);
					if (way == 1) {
						window.location.replace('preferentialShopStat.html?sign=2&click=2&hierarchy=2');
					} else if (way == 2) {
				...
				$('#selectbtn').unbind('click').bind('click', function () {
					...
					_self.StoredData(startdate, endDate, 'all', 'all');
				});
				...
				$('#tbodys').delegate('tr', 'click', function(event) {
                    ...
                    // 点击店铺明细
                    if (type == 'shopDetailed') {

						// 如果是单个日期并且是第一层点击店铺明细
						if ((sign == 1 || sign == 2) && hierarchy == 1){
							var preferentstat = {
								'start_date': date,
								'end_date': date,
								'shop_ids': 'all',
							...
							window.location.replace('preferentialShopStat.html?sign=1&click=2&hierarchy=2');
						}
						
						// 如果是多个日期，并且是第二层点击店铺明细
						if (sign == 2 && hierarchy == 2) {
							var preferentstat = {
								'start_date': date,
								'end_date': date,
							...
							window.location.replace('preferentialShopStat.html?sign=2&click=2&hierarchy=3');
						}
					...
					if (type == 'storedDetailed') {// 点击优惠方案明细
						...
						if (sign == 2 && hierarchy == 2) {
							...
							Cache.set('preferentstat',preferentstat);
							// url后面带的参数sign=2多个日期，click=2点击的上面的，hierarchy=3点击之后是第三层
							window.location.replace('preferentialPlanStat.html?sign=2&click=2&hierarchy=3');
						}
			...
			StoredList: function (data) {
				...
				for (var i in data) {
					content += '<tr class="total-tr" promo-id="'+data[i].promo_ids+'" shop-id="'+data[i].shop_id+'">'+
									...
									'<td class="total-caozuo clearfix">'+
										...
										(sign == 2 && hierarchy == 2 && way == 1 ?
										'<span>'+
											'<input type="button" value="优惠方案明细" data-type="storedDetailed" class="stores-caozuo-btn">'+
										'</span>':'')+
										// 如果是多个日期并且是第二层并且上一次点击的是优惠方案，就显示店铺明细
										(sign == 2 && hierarchy == 2 && way == 2 ?
										'<span>'+
											'<input type="button" value="店铺明细" data-type="shopDetailed" class="stores-caozuo-btn">'+
										'</span>':'')+

										// 如果是单个日期或者多个日期并且是第一层,就显示店铺明细和优惠方案明细
										((sign == 1 || sign == 2) && hierarchy == 1 ?
										'<span>'+
											'<input type="button" value="店铺明细" data-type="shopDetailed" class="stores-caozuo-btn">'+
										'</span>'+
										'<span>'+
				...
				content += '<tr class="total-tr">'+
								...
								'<td class="total-caozuo clearfix">'+
									...
									(sign == 2 && hierarchy == 2 && way == 2 ?
									'<span>'+
										'<input type="button" value="店铺明细" id="totalShopDetailed" class="stores-caozuo-btn">'+
									'</span>':'')+
				...
				$('#tbodys').html(content);
				// 总计店铺明细
				$('#totalShopDetailed').unbind('click').bind('click', function () {

						// 如果是第一层点击总计旁边的店铺，是多个日期
						if (hierarchy == 1) {
							var preferentstat = {
								'start_date': stData,
								'end_date': enData,
							...
							window.location.replace('preferentialShopStat.html?sign=2&click=1&hierarchy=1');
						}

						// 如果是多个日期，并且是第二层点击总计旁边的店铺明细
						if (sign == 2 && hierarchy == 2) {
							var preferentstat = {
								'start_date': stData,
								'end_date': enData,
								'shop_ids': 'all',
								'promo_ids': stoData.promor_ids,
								'type': 1,
								'name': stoData.name
							}
							Cache.set('preferentstat',preferentstat);
